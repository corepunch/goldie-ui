...
int main(int argc, char *argv[]) {
    ...
    // Test 2: evStatusBar message is defined
    TEST_ASSERT(evStatusBar > 0, "evStatusBar message is defined");
    ...
    if (!ui_init_graphics(UI_INIT_DESKTOP|UI_INIT_TRAY, "StatusBar Test", 320, 240)) {
        ...
        // Test 8: Send evStatusBar message
        const char *test_text = "Test Status";
        send_message(win, evStatusBar, 0, (void*)test_text);
        TEST_ASSERT(strcmp(win->statusbar_text, test_text) == 0, "evStatusBar updates statusbar_text");
        TEST_PASS("evStatusBar message handler works correctly");
        
        // Test 9: statusbar_text truncation
        char long_text[100];
        memset(long_text, 'X', sizeof(long_text) - 1);
        long_text[sizeof(long_text) - 1] = '\0';
        send_message(win, evStatusBar, 0, (void*)long_text);
