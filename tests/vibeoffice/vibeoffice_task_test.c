...
static void test_two_desks_finish_independently(void) {
  ...
  ASSERT_TRUE(vibe_task_submit(&killed, 1, "hello one", error, sizeof(error)));
  ASSERT_TRUE(vibe_task_submit(&healthy, 2, "hello \"two\"", error, sizeof(error)));
  ...
  ASSERT_STR_EQUAL(second.output, "reply:hello \"two\"");
