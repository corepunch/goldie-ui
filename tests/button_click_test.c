...
// Parent window procedure that receives WM_COMMAND from button
...
void test_button_click_with_scaling(void) {
    ...
    // Post WM_LBUTTONDOWN message to button
    ...
    ASSERT_TRUE(test_env_was_message_sent(WM_LBUTTONDOWN));
    
    // Now post WM_LBUTTONUP to trigger the button click
    test_env_post_message(button, WM_LBUTTONUP, MAKEDWORD(button_center_x, button_center_y), NULL);
    repost_messages();
    
    // Verify LBUTTONUP was tracked
    ASSERT_TRUE(test_env_was_message_sent(WM_LBUTTONUP));
    
    // Verify WM_COMMAND with BN_CLICKED was sent to parent
