...
typedef struct { char *v[9]; } attrs_t;
enum { A_NAME, A_TEXT, A_FLAGS, A_HA, A_VA, A_ORIENT, A_SPACING, A_FONT, A_COLOR };
...
static const field_type_t kFieldTypes[] = {
  { "integer", "int",    "DB_TYPE_INT",    0 },
  { "int",     "int",    "DB_TYPE_INT",    0 },
  { "string",  "char",   "DB_TYPE_STRING", 256 },
  { "boolean", "bool",   "DB_TYPE_BOOL",   0 },
  { "bool",    "bool",   "DB_TYPE_BOOL",   0 },
  { "float",   "float",  "DB_TYPE_FLOAT",  0 },
  { "real",    "float",  "DB_TYPE_FLOAT",  0 },
  { "double",  "double", "DB_TYPE_DOUBLE", 0 },
};
...
static void read_control_attrs(xmlNodePtr n, attrs_t *a) {
  static const struct { int slot; const char *name, *alias; } cfg[] = {
    ...
    {A_FONT, "font", NULL}, {A_COLOR, "color", NULL},
  };
  memset(a, 0, sizeof(*a));
...
static void emit_controls(FILE *f, xmlNodePtr parent, const char *form, const char *parent_id, int *count) {
  int ordinal = 0;
  EACH_ELEMENT(c, parent) if (is_control(parent, c)) {
    ...
    snprintf(flags, sizeof(flags), "(%s)%s", nz(a.v[A_FLAGS], "0"), enum_parse_token(a.v[A_ORIENT], kOrient, ARRAY_LEN(kOrient), WINDOW_STACK_VERTICAL) & WINDOW_STACK_HORIZONTAL ? " | WINDOW_STACK_HORIZONTAL" : "");
    ...
    (*count)++;
    emit_controls(f, c, form, id, count);
    free_attrs(&a);
  }
}
...
static bool emit_form(FILE *f, xmlNodePtr form, const char *prefix) {
  ...
  int count = 0; emit_controls(f, form, form_id, "0", &count); LINE("};\n\n");
