...
void layout_measure_window(window_t *win, layout_measure_t *m) {
  ...
  if (is_grid_layout(win)) {
    ...
    for (int col = 0; col < cols; col++) {
      if (col > 0) desired_w += gap;
      desired_w += col_w[col];
    }
    for (int row = 0; row < rows; row++) {
      if (row > 0) desired_h += gap;
      desired_h += row_h[row];
    }
    free(col_w);
    free(row_h);
...
static void create_form_children_flat(window_t *parent, const form_ctrl_def_t *children,
  ...
  for (int i = 0; i < child_count; i++) {
    ...
    if ((cp == win_stackview || cp == win_gridview) && child->v_align == LAYOUT_ALIGN_STRETCH)
      child->v_align = LAYOUT_ALIGN_START;
