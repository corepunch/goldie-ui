...
3. Resize proportionally into a transparent **48x48 RGBA PNG** using `contain`;
...
The Icon control may display these smaller than 48x48 to fit an agent's artefact
strip. Keeping the source files at 48x48 gives the UI enough resolution while
keeping the complete set lightweight.
...
Status icons use the same 3D PNG preparation but are not draggable artefacts.
They render immediately before the agent name, keeping the right-hand strip
reserved for transferable work objects.

| Agent state | Fluent Emoji asset |
|---|---|
| Available | Check mark button |
| Busy | Hourglass not done |
| Pending | Timer clock |
| Error | Warning |
